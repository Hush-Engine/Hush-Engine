...
EFileExtension AssetCooker::ExtensionFromPath(const std::filesystem::path &path)
{
	...
	if (ext == "PNG") return EFileExtension::PNG;
	if (ext == "JPG" || ext == "JPEG") return EFileExtension::JPEG;
	if (ext == "SLANG") return EFileExtension::SLANG;
	if (ext == "HMETA") return EFileExtension::HMETA;
	if (ext == "HASSET") return EFileExtension::HASSET;
	if (ext == "HSHADER") return EFileExtension::HSHADER;
	if (ext == "HUSHPAK") return EFileExtension::HUSHPAK;
	return EFileExtension::UNKNOWN;
}
Result<std::vector<std::byte>, ECookError>
AssetCooker::CookToBlob(std::span<const std::byte> input, EFileExtension sourceExtension, const HMeta &meta,
							  const CookContext &ctx)
{
	...
	asset.header.contentHash = Hashing::Fnv1a(
	...
	if (meta.compression == ECompressionFormat::Zstd)
	{
		...
		size_t compressedSize = ZSTD_compress(compressed.data(), compressedBound,
...
Result<void, ECookError>
AssetCooker::CookDirectory(const std::filesystem::path &contentDir, ECompressionFormat compression)
{
	std::error_code ec;
...
Result<void, ECookError>
AssetCooker::BuildPak(std::span<const PakInput> inputs, IFile &out)
{
