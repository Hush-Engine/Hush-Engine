...
namespace Hush
{
	...
	public:
		...
		Result<std::vector<std::byte>, ECookError> CookToBlob(std::span<const std::byte> input,
															  EFileExtension sourceExtension, const HMeta &meta,
		...
		/// Build a .hushpak from a directory that already contains .hmeta sidecars and
		/// .hcooked/{id}.hasset outputs. The virtual path for each cooked blob is recovered
		/// by scanning the .hmeta files (source path minus .hmeta extension, relative to
		/// contentDir).
		Result<void, ECookError> BuildPakFromCookedDir(const std::filesystem::path &contentDir, IFile &out);
