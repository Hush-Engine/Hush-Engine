...
static void GenerateMips(const std::vector<std::byte> &srcRGBA, int w, int h,
						 std::vector<std::byte> &outMipData, uint32_t &outMipCount)
{
	...
	while (mipW > 1 || mipH > 1)
	{
		...
		stbir_resize_uint8_srgb(
			reinterpret_cast<const unsigned char *>(prevData), mipW, mipH, 0,
										...
										const std::string &gpuCompression)
{
	...
	enum class BCNFormat { BC1, BC3, BC5, BC7 };
	BCNFormat fmt = BCNFormat::BC7;
	if (gpuCompression == "BC1") fmt = BCNFormat::BC1;
	else if (gpuCompression == "BC3") fmt = BCNFormat::BC3;
	else if (gpuCompression == "BC5") fmt = BCNFormat::BC5;
	else if (gpuCompression == "BC7") fmt = BCNFormat::BC7;
	else return std::vector<std::byte>(rgba.begin(), rgba.end()); // unsupported, pass through

	// Determine block size
	const size_t numBlocks = (static_cast<size_t>(w + 3) / 4) * (static_cast<size_t>(h + 3) / 4);
	size_t blockSize = 16; // BC7 = 16 bytes/block
	if (fmt == BCNFormat::BC1) blockSize = 8;
...
Result<ImageCooker::CookResult, ECookError>
ImageCooker::Cook(std::span<const std::byte> input, const HMeta &meta, const CookContext &ctx)
{
	int width{};
	int height{};
	int channels{};
	...
	stbi_uc *imageData = stbi_load_from_memory(
		reinterpret_cast<const stbi_uc *>(input.data()),
	...
	if (meta.texture.gpuCompression != "none")
	{
		...
		if (meta.texture.gpuCompression == "BC1") finalFormat = EAssetFormat::DXT1;
		else if (meta.texture.gpuCompression == "BC3") finalFormat = EAssetFormat::DXT5;
		else if (meta.texture.gpuCompression == "BC5") finalFormat = EAssetFormat::BC5;
		else if (meta.texture.gpuCompression == "BC7") finalFormat = EAssetFormat::BC7;
	}
	else
	{
