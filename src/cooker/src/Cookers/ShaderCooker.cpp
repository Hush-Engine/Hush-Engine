...
static void DetectEntryPoints(std::string_view source, std::vector<Graphics::ShaderEntryPointRequest> &outEntryPoints)
{
...
Result<ShaderCooker::CookResult, ECookError>
ShaderCooker::Cook(std::span<const std::byte> input, const HMeta &meta, const CookContext &ctx)
{
...

	{
		...
		compileResult = compiler->CompileFromSource(source, ctx.sourceVPath, entryPoints);
	}
