...
namespace Hush
{
	...
	bool CookedDirectory::HandleFileEvent(const std::filesystem::path &path, bool isAdded, bool isRemoved,
										  bool isModified)
	{
		...
		if (isAdded || isModified)
		{
			...
			LogFormat(ELogLevel::Info, "CookedDirectory: cooked {} -> {}", path.filename().string(),
					  cookedPath.string());
			return true;
		}
