...
namespace Hush
{
	...
	std::optional<HushPak> HushPak::Read(std::span<const std::byte> data)
	{
		...
		if (dirCount > 0)
		{
			const size_t dirBytes = dirCount * sizeof(PakEntry);
			if (hdr.directoryOffset > data.size() || dirBytes > data.size() - static_cast<size_t>(hdr.directoryOffset))
			{
				return std::nullopt;
			}
			std::memcpy(pak.directory.data(), data.data() + static_cast<size_t>(hdr.directoryOffset), dirBytes);

			// Read string table
			if (hdr.stringTableSize > 0)
			{
				if (hdr.stringTableOffset > data.size() ||
					hdr.stringTableSize > data.size() - static_cast<size_t>(hdr.stringTableOffset))
				{
					return std::nullopt;
				}
				pak.stringTable.resize(static_cast<size_t>(hdr.stringTableSize));
				std::memcpy(pak.stringTable.data(), data.data() + static_cast<size_t>(hdr.stringTableOffset),
							static_cast<size_t>(hdr.stringTableSize));
			}
		}
	...
	std::span<const std::byte> HushPak::EntryData(const PakEntry &entry, std::span<const std::byte> pakData) const
	{
		...
		const size_t dataSize = static_cast<size_t>(entry.dataSize);
		if (dataOffset > pakData.size() || dataSize > pakData.size() - dataOffset)
		{
			return {};
		}

		const size_t dataStart = static_cast<size_t>(header.dataOffset) + dataOffset;
		if (dataStart > pakData.size() || dataSize > pakData.size() - dataStart)
		{
			return {};
		}
		return pakData.subspan(dataStart, dataSize);
	}
