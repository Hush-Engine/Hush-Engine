...
Result<std::string, Serialization::ESerializationError> HMeta::ToJson() const
{
	...
	writer.Key("version"); writer.Uint(this->version);
	writer.Key("id"); writer.Uint(this->id);
	writer.Key("sourceHash"); writer.Uint64(this->sourceHash);
	writer.Key("sourceMTime"); writer.Uint64(this->sourceMTime);
	writer.Key("assetType"); writer.String(this->assetType.c_str());
	writer.Key("outputFormat"); writer.Uint(static_cast<uint32_t>(this->outputFormat));
	writer.Key("compression");
	switch (this->compression)
	{
	case ECompressionFormat::Zstd: writer.String("zstd"); break;
	default: writer.String("none"); break;
	}
	...
	// Texture settings
	writer.Key("sRGB"); writer.Bool(this->texture.sRGB);
	writer.Key("generateMipmaps"); writer.Bool(this->texture.generateMipmaps);
	writer.Key("gpuCompression"); writer.String(this->texture.gpuCompression.c_str());
	writer.Key("maxSize"); writer.Uint(this->texture.maxSize);
	...
	// Model settings
	writer.Key("importMaterials"); writer.Bool(this->model.importMaterials);
	writer.Key("generateLods"); writer.Bool(this->model.generateLods);
	writer.Key("scaleFactor"); writer.Double(static_cast<double>(this->model.scaleFactor));
