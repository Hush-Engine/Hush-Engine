...
namespace Hush
{
    ...
    class ScriptingManager
    {
      public:
        ...
        /// <param name="m_targetAssembly">The desired assembly this manager is going to invoke methods from, THIS
        /// SHOULD BE CONSTANT WHEN POSSIBLE</param>
        ScriptingManager(std::shared_ptr<DotnetHost> host, std::string_view targetAssembly);

        template <class R, class... Types>
        R InvokeCSharpWithReturn(const char *targetNamespace, const char *targetClass, const char *fnName,
                                 Types... args)
        {
            // TODO: Consider caching the functions in memory to a map so that we don't have to constantly load them
            // every time
      ...
      private:
        ...
        template <class... Types> int GetMethodFromCS(const char *fullClassPath, const char *fnName, void **outMethod)
        {
            ...
            int rc =
                functionGetter(classPath, targetFunction, UNMANAGEDCALLERSONLY_METHOD, nullptr, nullptr, outMethod);
            return rc;
        }
    };
} // namespace Hush
