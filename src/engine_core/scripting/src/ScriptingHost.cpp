...
void Hush::ScriptingHost::FetchSystemsIntoCache() {
	HUSH_ASSERT(this->m_getSystemCountFn != nullptr, "Function pointer to get system count is not initialized, forgot to call ScriptingHost::Initialize?");
	...
	HUSH_ASSERT(this->m_getAvailableSystemsFn != nullptr, "Function pointer to get available systems is not initialized, forgot to call ScriptingHost::Initialize?");
...
Hush::Result<uintptr_t, Hush::ScriptingHost::EError> Hush::ScriptingHost::CreateSystem(const ScriptingSystemInfo& systemInfo) {
	HUSH_ASSERT(this->m_instantiateSystemFn != nullptr, "Function pointer to create systems is not initialized, forgot to call ScriptingHost::Initialize?");
	...
	HUSH_COND_FAIL_MSG_V(sysPtr != nullptr, EError::SystemNotFound, "Could not find system named {} in the scripting host", systemInfo.name);
