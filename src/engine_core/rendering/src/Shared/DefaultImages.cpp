...
void Hush::DefaultImageProvider::CreateDefaultImages(IRenderer *renderer)
{
	...
	this->m_blackImage =
		renderer->CreateImage(&storedBlack, defaultExtent, Color::EFormat::RGBA8Unorm, vkImgUsageSampledBit);
	uint32_t storedWhite = white.ToColor32();

	this->m_whiteImage =
		renderer->CreateImage(&storedWhite, defaultExtent, Color::EFormat::RGBA8Unorm, vkImgUsageSampledBit);
	...
	renderer->AddToDeletionQueue([this, &renderer]() {
		...
		renderer->DestroyImage(&this->m_transparentImage);
	});
}
