...
namespace Hush
{
		  ...
		  m_watcher(nullptr, [](void *) {})
	{
		...
		try
		{
			auto *fw = new FW(m_rootDir.string(), [this](const std::string &pathStr, const filewatch::Event eventType) {
			...
			m_watcher = std::unique_ptr<void, void (*)(void *)>(
				fw, [](void *p) { delete static_cast<filewatch::FileWatch<std::string> *>(p); });
	...
	void FileWatcher::OnFileChanged(const std::filesystem::path &path, bool isAdded, bool isRemoved,
									bool /*isModified*/)
	{
		FileWatchEvent::Type type;
		if (isAdded)
			type = FileWatchEvent::Type::Added;
		else if (isRemoved)
			type = FileWatchEvent::Type::Removed;
		else
			type = FileWatchEvent::Type::Modified;
