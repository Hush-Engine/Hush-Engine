...
void CookerService::ImportFile(const std::filesystem::path &sourcePath)
{
	...
	{
		LogFormat(ELogLevel::Error, "CookerService: failed to copy {} to {}: {}", sourcePath.string(), destPath.string(),
				  ec.message());
		return;
	}
	...
	LogFormat(ELogLevel::Info, "CookerService: imported {} -> {}", sourcePath.filename().string(), metaPath.string());
