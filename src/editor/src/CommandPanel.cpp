...
void Hush::CommandPanel::FindEntityPopup(const char *overrideLabel)
{
	...
	Query<WorldTransform, Entity::Name> query = this->m_activeScene->CreateQuery<WorldTransform, Entity::Name>();
	std::vector<std::string> entityNames;
	std::string_view searchEntityName(this->m_searchInputText);
	entityNames.reserve(query.begin().Size());
	query.Each([&entityNames, &searchEntityName, this](Entity &entity, WorldTransform &transform, Entity::Name& name) {
