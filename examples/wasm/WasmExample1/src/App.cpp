...
// WGSL shader source – used directly since Slang is not available on Wasm.
static constexpr const char *FULLSCREEN_SHADER_WGSL = R"(
struct Uniforms {
    mvp: mat4x4<f32>,
    tintColor: vec4<f32>,
    time: f32,
...
private:
	bool InitShaderResources()
	{
		...
		// Build shader module descriptors directly from WGSL source
		// (Slang is not available on Wasm, so we bypass ShaderCompiler entirely)
		ShaderModuleDescriptor vsDesc{};
		vsDesc.stage = EShaderStage::Vertex;
		vsDesc.entryPoint = "vertexMain";
		vsDesc.bytecode.content = ShaderBytecode::TextContent{.sourceText = std::string(FULLSCREEN_SHADER_WGSL)};
		vsDesc.debugName = "fullscreen_texture_vs";

		ShaderModuleDescriptor fsDesc{};
		fsDesc.stage = EShaderStage::Fragment;
		fsDesc.entryPoint = "fragmentMain";
		fsDesc.bytecode.content = ShaderBytecode::TextContent{.sourceText = std::string(FULLSCREEN_SHADER_WGSL)};
		fsDesc.debugName = "fullscreen_texture_fs";

		m_vertexShader.CreateResource(vsDesc, device);
		m_fragmentShader.CreateResource(fsDesc, device);

		Hush::LogFormat(Hush::ELogLevel::Info, "[ExampleApp] WGSL shader modules created.");
