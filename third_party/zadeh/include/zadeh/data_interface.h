...
template <> vector<CandidateString> init(const size_t len, const std::allocator<CandidateString> &alloc) {
  auto out = vector<CandidateString>(alloc);
  out.reserve(len);
  return out;
}

template <> inline CandidateString get_at(const vector<CandidateString> &candidates, const size_t iCandidate) {
  return candidates[iCandidate];
}

template <> size_t get_size(const vector<CandidateString> &candidates) { return candidates.size(); }
...
template <> inline vector<CandidateString> get_ref(const vector<CandidateString> &vect) { return vect; }
