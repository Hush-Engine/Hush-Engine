...
#include <thread>
#include <limits>
#include <zadeh/path_scorer.h>
#include <zadeh/scorer.h>
...
struct CandidateScore {
  ...
  bool operator<(const CandidateScore &other) const noexcept { return score > other.score; }
};

using CandidateScoreVector = std::vector<CandidateScore>;

void filter_internal(const std::vector<CandidateString> &candidates, size_t start_index, const Element &query,
                     const Options &options, size_t max_results, CandidateScoreVector &results) {
...
std::vector<CandidateIndex> filter(const vector<std::vector<CandidateString>> &candidates,
  ...
  for (size_t i = 1; i < candidates_size; i++) {
    ...
    threads.emplace_back(filter_internal, ref(candidates[i]), start_index, ref(query), ref(options),
                         max_results, ref(results[i])); // inbounds
  }
